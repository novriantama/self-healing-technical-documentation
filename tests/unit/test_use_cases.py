...
def test_get_changed_code_chunks():
    ...
    use_case = GetChangedCodeChunksUseCase(git_provider, code_parser)

    import os
    import tempfile

    with tempfile.TemporaryDirectory() as tmpdir:
        # Create math.py file to satisfy path exists check
        with open(os.path.join(tmpdir, "math.py"), "w") as f:
            f.write("# dummy code")

        changed = use_case.execute("fake diff text", tmpdir)
